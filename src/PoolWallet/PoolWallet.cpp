...
bool pool_wallet::confirmTransaction(TransferCommand cmd, bool multiAddress) {
  ...
  if (cmd.fee == 10) {
    feeString = "0.1 AGIO (minimum)";
  } else {
    feeString = m_currency.formatAmount(cmd.fee) + " AGIO";
  }
  ...
  if (!multiAddress) {
    std::cout << "You are sending " << m_currency.formatAmount(cmd.dsts[0].amount)
              << " AGIO, with a fee of " << feeString << std::endl
    ...
    for (auto destination : cmd.dsts) {
      std::cout << "You are sending " << m_currency.formatAmount(destination.amount)
                << "AGIO" << std::endl << "FROM: " << walletName << std::endl
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string>& args)
{
  ...
  for (size_t i = 0; i < ourTransaction.outputs.size(); ++i)
  {
    ...
    if (targetPubKey.key == outputPublicKey)
    {
      ...
      std::string agio = m_currency.formatAmount(amount);

      sum += amount;

      found = true;

      logger(INFO, GREEN) << "The transaction output of " << agio << " AGIO belongs to you!";
    }
  }
  ...
  {
    std::string agio = m_currency.formatAmount(sum);

    logger(INFO, GREEN) << "Outputs totalling " << agio << " AGIO were sent to your wallet!";
  }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Agio v" << PROJECT_VERSION << " Poolwallet" << std::endl;
