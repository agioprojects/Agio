/*
Copyright (C) 2018, The Agio developers
         ...
         Config &config)
{
    ...
    do
    {
        std::cout << InformationMsg("Agio v"
    ...
    {
        std::cout << WarningMsg("It looks like Agiod isn't open!")
                  << std::endl << std::endl
                  << WarningMsg("Ensure Agiod is open and has finished "
                                "initializing.")
                  << std::endl
                  << WarningMsg("If it's still not working, try restarting "
                                "Agiod. The daemon sometimes gets stuck.") 
                  << std::endl
                  << WarningMsg("Alternatively, perhaps d can't "
...
std::shared_ptr<WalletInfo> createViewWallet(CryptoNote::WalletGreen &wallet)
{
    ...
    while (true)
    {
        ...
        else if (address.substr(0, 4) != "Tgio")
        {
            std::cout << WarningMsg("Invalid address! It should start with "
                                    "\"Tgio\"!") << std::endl;
                                                        ...
                                                        address))
        {
            std::cout << WarningMsg("Failed to parse AGIO address! Ensure you "
...
void help(bool viewWallet)
{
    std::cout << "Available commands:" << std::endl
              ...
              << "Display how much AGIO you have" << std::endl
    ...
    if (viewWallet)
    {
        std::cout << InformationMsg("Please note you are using a view only "
                                    "wallet, and so cannot transfer AGIO.")
                  << std::endl;
    }
    else
    {
                           ...
                           CryptoNote::INode &node)
{
    ...
    /* Couldn't get timestamp, maybe old node or Agiod closed */
    if (blockTime != "")
    {
                         ...
                         std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    if (localHeight != remoteHeight)
    {
        std::cout << "Your Agiod isn't fully synced yet!" << std::endl
    ...
    while (walletHeight < localHeight)
    {
        ...
        if (tmpWalletHeight == walletHeight)
        {
            ...
            {
                std::string warning =
                    "Syncing may be stuck. Try restarting Agiod.\n"
...
ColouredMsg getPrompt(std::shared_ptr<WalletInfo> &walletInfo)
{
    ...
    return InformationMsg("[AGIO " + shortName + "]: ");
}

void connectingMsg()
{
    std::cout << std::endl << "Making initial contact with Agiod."
              << std::endl
...
void viewWalletMsg()
{
    std::cout << InformationMsg("Please remember that when using a view wallet "
              ...
              << std::endl << "This means if you received 100 AGIO and then "
              << "sent 50 AGIO, your balance would appear to still be AGIO "
              << "AGIO." << std::endl
