        ...
        catch (const std::system_error &e)
        {
            ...
            else if (errMsg == "Network error")
            {
                std::cout << WarningMsg("Couldn't connect to the network to "
                          ...
                          << "Ensure Agiod or the remote node you are "
...
Maybe<uint64_t> getFee()
{
    while (true)
    {
        ...
        std::cout << std::endl 
                  ...
                  << std::endl
                  << "Hit enter for the default fee of 0.1 AGIO: ";
...
Maybe<uint64_t> getTransferAmount()
{
    while (true)
    {
        ...
        std::cout << std::endl
                  << InformationMsg("How much AGIO do you want to send?: ");
...
bool parseFee(std::string feeString)
{
    ...
    {
        std::cout << WarningMsg("Fee must be at least 0.1 AGIO!") << std::endl;
        return false;
    }
...
bool parseAddress(std::string address)
{
    ...
    else if (expectedPrefix == 0x32849f && address.substr(0, 4) != "Tgio")
    {
        std::cout << WarningMsg("Invalid address! It should start with Tgio!")
                  << std::endl << std::endl;
        return false;
    }
...
bool parseAmount(std::string amountString)
{
    ...
    if (!parseAmount(amountString, amount))
    {
        std::cout << WarningMsg("Failed to parse amount! Ensure you entered "
                                "the value correctly.")
                  << std::endl
                  << "Please note, the minimum you can send is 0.00000001 AGIO,"
                  << std::endl
